...
const Admin = () => {
  ...
  const approveRequest = async (id) => {
    try {
      await axios.post('http://localhost:6001/approve-operator', { id }).then((response) => {
  ...
  const rejectRequest = async (id) => {
    try {
      await axios.post('http://localhost:6001/reject-operator', { id }).then((response) => {
    ...
    <>
      <div className="admin-page">
        ...
        <div className="admin-requests-container">
          <h3 className="admin-requests-title">New Operator Applications</h3>
          <div className="admin-requests">
            {users.length === 0 ? (
              <p style={{color: "white"}}>No new requests..</p>
            ) : (
              <>
                {users.map((user) => (
